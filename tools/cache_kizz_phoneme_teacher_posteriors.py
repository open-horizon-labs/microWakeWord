...
def compact_vocabulary(
    tokenizer: Any, *, phrase_id: str = HI_FI_KIZZ.phrase_id
) -> dict[str, Any]:
    ...
    return {
        ...
        "teacher_token_ids": {
            phone: token_id for phone, token_id in zip(required_phones, phone_ids)
        },
        ...
        "collision_paths": {
            ...
            for transcript, path in zip(
                phrase.collision_transcripts, phrase.collision_phones
            )
        },
...
def teacher_timing_metadata(
    model: Any, *, sample_rate: int = TARGET_SAMPLE_RATE
) -> dict[str, Any]:
    ...
    if (
        ...
        or any(value <= 0 for value in kernels + strides)
    ):
...
def compact_log_posteriors(
    logits: np.ndarray, vocabulary: Mapping[str, Any]
) -> np.ndarray:
    ...
    for compact_id, phone in enumerate(vocabulary["phone_tokens"], start=1):
        output[:, compact_id] = log_probs[
            :, int(vocabulary["teacher_token_ids"][phone])
        ]
    other_ids = [
        index
        for index in range(values.shape[1])
        if index not in selected and index != blank
    ]
    output[:, int(vocabulary["other_compact_id"])] = (
        _logsumexp(log_probs[:, other_ids], axis=1) if other_ids else -np.inf
    )
    return output


def _model_logits(
    model: Any, processor: Any, waveform: np.ndarray, device: Any
) -> np.ndarray:
...
def _load_audio(path: Path) -> tuple[np.ndarray, int]:
    try:
        ...
        raise RuntimeError(
            "soundfile is required to cache teacher posteriors"
        ) from error
...
def cache_manifest(
    ...
    *,
    ...
    vocabulary_override: Mapping[str, Any] | None = None,
    processor_vocab_sha256_override: str | None = None,
) -> dict[str, Any]:
    rows = _rows(manifest_path)
    vocabulary = (
        dict(vocabulary_override)
        if vocabulary_override
        else compact_vocabulary(tokenizer, phrase_id=phrase_id)
    )
    ...
    for index, row in enumerate(rows):
        ...
        if sample_rate != TARGET_SAMPLE_RATE:
            raise ValueError(
                f"audio must be {TARGET_SAMPLE_RATE} Hz: {path} ({sample_rate})"
            )
        compact = compact_log_posteriors(
            _model_logits(model, processor, waveform, device), vocabulary
        )
    ...
    matrix = (
        ...
        if arrays
        else np.empty((0, len(vocabulary["tokens"])), dtype=np.float32)
    )
    ...
    metadata = {
        ...
        "processor_vocab_sha256": (
            processor_vocab_sha256_override
            if processor_vocab_sha256_override
            else processor_vocab_hash(tokenizer)
        ),
    ...
    data_hash = _sha256_bytes(
        matrix.tobytes(order="C")
        + offsets_array.tobytes(order="C")
    ...
    with tempfile.NamedTemporaryFile(
        prefix=output_prefix.name, suffix=".npz", dir=output_prefix.parent, delete=False
    ) as temp:
...
def load_cache(
    ...
    *,
    ...
    if (
        ...
        and metadata.get("model", {}).get("weights_sha256") != expected_weights_sha256
    ):
        raise ValueError("stale cache: teacher weights differ")
    if (
        expected_processor_vocab_sha256
        and metadata.get("processor_vocab_sha256") != expected_processor_vocab_sha256
    ):
    ...
    with np.load(prefix.with_suffix(".npz"), allow_pickle=False) as loaded:
        arrays = {
            "log_posteriors": np.asarray(loaded["log_posteriors"]),
            "offsets": np.asarray(loaded["offsets"]),
        }
    expected_hash = _sha256_bytes(
        arrays["log_posteriors"].tobytes(order="C")
        + arrays["offsets"].tobytes(order="C")
        + _canonical_json(
            {key: value for key, value in metadata.items() if key != "cache_sha256"}
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output", type=Path, required=True, help="cache prefix; writes .npz and .json"
    )
    parser.add_argument(
        "--phrase-id", choices=tuple(sorted(WAKE_PHRASES)), default=HI_FI_KIZZ.phrase_id
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Reuse a hash-bound compact vocabulary from an existing posterior "
            "cache, avoiding an unnecessary phonemizer runtime dependency."
        ),
    )
    ...
    processor_vocab_sha256_override = None
    if args.vocabulary_cache:
        vocabulary_metadata = json.loads(
            args.vocabulary_cache.with_suffix(".json").read_text()
        )
        if any(
            vocabulary_metadata.get("model", {}).get(key) != teacher_model[key]
            for key in ("revision", "weights_sha256")
        ):
            parser.error("--vocabulary-cache uses a different qualified teacher")
        vocabulary_override = vocabulary_metadata.get("vocabulary")
        processor_vocab_sha256_override = vocabulary_metadata.get(
            "processor_vocab_sha256"
        )
        if not vocabulary_override or not processor_vocab_sha256_override:
            parser.error("--vocabulary-cache lacks its vocabulary contract")
        import torch
        ...
        processor = Wav2Vec2FeatureExtractor.from_pretrained(
            teacher_model["id"], **common
        )
        device = torch.device(args.device)
        model = (
            Wav2Vec2ForCTC.from_pretrained(teacher_model["id"], **common)
            .to(device)
            .eval()
        )
        tokenizer = None
    ...
    metadata = cache_manifest(
        ...
        processor_vocab_sha256_override=processor_vocab_sha256_override,
    )
