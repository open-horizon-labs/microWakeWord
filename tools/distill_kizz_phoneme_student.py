...
from microwakeword.kizz_feature_archive import decode_frontend_features, open_feature_archive
...
def student_architecture_contract(contract: dict) -> dict:
...
def expected_schedule_counts(
    ...
    if steps < 1 or batch_size < 8 or batch_size % 8:
        raise ValueError("steps must be positive and batch size must be a positive multiple of eight")
    half = batch_size // 2
    ...
    groups = ("public_speech", "kizz_control_phonetic_collision", "device_collision", "no_speech")
    negatives = Counter(groups[(step * half + offset) % len(groups)] for step in range(steps) for offset in range(half))
    return providers, negatives, positive_variant_providers


def nested_variant_provider_counts(counts: Counter[tuple[str, str]]) -> dict[str, dict[str, int]]:
    return {
        variant: {
...
def positive_indices_by_provider(rows: list[dict], variant: str) -> dict[str, np.ndarray]:
    ...
    if unknown:
        raise ValueError(f"device positives use unapproved providers: {sorted(unknown)}")
    return result
...
def collision_path_supervision(
    ...
    for index, row in enumerate(rows):
        ...
        if source is None or source.get("semantic_label") != "phonetic_collision":
            raise ValueError("phonetic collision row is not bound to its source manifest")
        text = str(source.get("render_text", ""))
        if text not in COLLISION_TEXT_TO_PATH:
            raise ValueError(f"phonetic collision has no supervision contract: {text!r}")
        path_name = COLLISION_TEXT_TO_PATH[text]
        if path_name is None:
            generic_texts[text] += 1
            continue
        if path_name not in path_indexes:
            raise ValueError(f"collision supervision path is not in decoder: {path_name}")
...
def checkpoint_binding(output: Path, best_step: int, best_key: tuple[float, float]) -> dict:
...
def require_cache_binding(cache_meta: dict, teacher_qualification: Path, teacher_manifest_sha256: str) -> None:
    ...
    if embedded.get("sha256") not in (active_hash, source_hash):
        raise ValueError("teacher posterior cache is not bound to the active qualification lineage")
    ...
    if cache_meta.get("manifest_sha256") != teacher_manifest_sha256:
        raise ValueError("teacher posterior cache is for a different distillation corpus")
...
def load_teacher_sequence_cache(
    ...
    *,
    ...
    if metadata.get("representation") != "qualified_teacher_original_resolution_clip_decisions":
    ...
    if (
        posterior.get("json_sha256")
        != sha256_file(posterior_prefix.with_suffix(".json"))
        or posterior.get("npz_sha256")
        != sha256_file(posterior_prefix.with_suffix(".npz"))
    ):
...
def load_student_window_cache(
    ...
    *,
    ...
    if (
        ...
        != "student_streaming_window_hard_mining"
    ):
        raise ValueError("unsupported student streaming-window cache")
    corpus = metadata.get("corpus", {})
    if (
        corpus.get("sha256") != sha256_file(corpus_json)
        or corpus.get("features_sha256") != sha256_file(features_path)
    ):
    ...
    if (
        scorer.get("algorithm") != "forward_sum_ctc"
        or scorer.get("window_lengths_frames")
        != list(WINDOW_LENGTHS_FRAMES)
...
def load_expanded_public_negatives(
    ...
    *,
    ...
    if (
        metadata.get("source_manifest", {}).get("sha256")
        != sha256_file(source_manifest)
        or metadata.get("continuous_lock", {}).get("sha256")
        != sha256_file(continuous_lock)
    ):
    ...
    if (
        ...
        or metadata.get("features", {}).get("sha256")
...
def require_teacher_gates(clip_report: Path, continuous_report: Path) -> tuple[dict, dict]:
    clip = json.loads(clip_report.read_text())
    continuous = json.loads(continuous_report.read_text())
    if clip.get("gate_scope") != "teacher_clip_and_anchor_prequalification" or clip.get("qualified") is not True:
    ...
    if continuous.get("gate_scope") != "untouched_continuous_qualification" or continuous.get("qualified") is not True:
        raise ValueError("teacher continuous qualification did not pass")
    bound_qualification_sha = continuous.get("teacher_qualification_sha256") or continuous.get("teacher_qualification", {}).get("report_sha256")
    ...
    for artifact_hash in ("weights_sha256", "config_sha256", "tokenizer_vocab_sha256"):
        if continuous.get("model", {}).get(artifact_hash) != clip.get("model", {}).get(artifact_hash):
            raise ValueError(f"continuous and clip reports bind different teacher {artifact_hash}")
    return clip, continuous


def map_ordered_targets(targets: np.ndarray, provenance: dict, contract: dict) -> np.ndarray:
    ...
    if not isinstance(phrase, dict) or phrase.get("phrase_id") != KIZZ_CONTROL.phrase_id:
    ...
    if provenance.get("states_per_phone") != expected_topology.states_per_phone:
        raise ValueError("overlay targets do not use the Kizz Control double-state contract")
    ...
    source_times = np.asarray(provenance.get("target_frame_times_seconds"), dtype=np.float64)
    ...
    if np.any(~np.isfinite(source_times)) or np.any(np.diff(source_times) <= 0):
        raise ValueError("overlay target timeline must be finite and strictly increasing")
    ...
    target_times = student_output_times_seconds(student_flags(len(contract["tokens"])), OUTPUT_FRAMES)
    ...
    for phone_index, token_id in enumerate(contract["canonical_path"]):
        for state_index in range(expected_topology.states_per_phone):
            state_to_token[expected_topology.phone_state_index(phone_index, state_index)] = int(token_id)
...
def require_overlay_parent_binding(
    ...
    if any(variants != expected_variants for variants in variants_by_parent.values()):
        raise ValueError("overlay parents do not each realize the four-variant contract")
...
class DistillationBatcher:
    def __init__(
        ...
        *,
        ...
        self.noise = [(name, open_feature_archive(path)) for name, path in noise_sources]
        ...
        self.overlay_positive = {
            provider: np.asarray([i for i, value in enumerate(overlay_providers) if value == provider], dtype=np.int64)
            for provider in APPROVED_PROVIDERS
        }
        if len(overlay_features) != len(overlay_targets) or len(overlay_features) != len(overlay_providers):
            raise ValueError("overlay feature, target, and provider counts differ")
        if self.teacher_sequence_targets.shape != (len(rows), 2):
            raise ValueError("teacher sequence targets must be [corpus, canonical/margin]")
        if self.collision_path_indexes.shape != (len(rows),):
            raise ValueError("collision path supervision must match the corpus")
        if any(values.shape != (len(rows),) for values in streaming_window_targets.values()):
        ...
        if any(
            ...
            raise ValueError("every approved provider needs clean, overlaid, and device positives")
        self.negative = {
            group: np.asarray([i for i in train if rows[i]["label"] == 0 and rows[i].get("source_group") == group], dtype=np.int64)
        ...
        for group, indexes in self.negative.items():
            ...
            hard_count = max(1, math.ceil(len(order) / 2))
            self.hard_negative[group] = np.asarray(
                order[:hard_count], dtype=np.int64
            )
    ...
    def batch(self, step: int):
        ...
        scoring_endpoints = np.full(
            self.batch_size, OUTPUT_FRAMES, dtype=np.int32
        )
        ...
        for slot in range(half):
            ...
            if variant == "overlay":
                ...
                source = self.device_positive if variant == "device" else self.clean_positive
                full_schedule = len(POSITIVE_VARIANTS) * len(APPROVED_PROVIDERS)
                use_hard_pool = variant == "clean" and (global_slot // full_schedule) % 2 == 0
                pool = self.hard_clean_positive[provider] if use_hard_pool else source[provider]
        ...
        for offset in range(half):
            ...
            if group == "no_speech":
                ...
                if group == "public_speech":
                    order_index = (
                        self.expanded_public_negative_count
                        % len(self.expanded_public_order)
                    )
                ...
                pool = self.hard_negative[group] if use_hard_pool else self.negative[group]
                index = int(rng.choice(pool))
                if use_hard_pool:
    ...
    *,
    ...
    if endpoints is None:
        ...
        tf.debugging.assert_equal(
            tf.shape(selected_endpoints)[0], batch,
    ...
    for frame_index in range(maximum_window):
        emissions = tf.gather(
            log_probs[:, frame_index, :], expanded_tokens, axis=1
        )
        initial = tf.where(
            initial_state_mask[None, :, :], emissions, negative
        )[:, None, :, :]
        from_one = tf.concat(
        ...
        from_two = tf.where(
            skip_allowed[None, None, :, :], from_two, negative
        )
        if algorithm == "max_add_ctc_viterbi":
    ...
    valid_windows = selected_endpoints[:, None] >= tf.constant(
        WINDOW_LENGTHS_FRAMES, dtype=tf.int32
    )[None, :]
...
def distillation_loss(
    ...
    *,
    ...
    student_log_probs = tf.nn.log_softmax(logits, axis=-1)
    teacher_probs = tf.exp(teacher_log_probs)
    kl = tf.reduce_mean(
        tf.reduce_sum(teacher_probs * (teacher_log_probs - student_log_probs), axis=-1),
        axis=-1,
    )
    teacher_loss = _masked_mean(kl, teacher_mask)
    valid = hard_targets >= 0
    safe = tf.where(valid, hard_targets, 0)
    hard = tf.keras.losses.sparse_categorical_crossentropy(safe, logits, from_logits=True)
    hard_loss = tf.math.divide_no_nan(tf.reduce_sum(hard * tf.cast(valid, hard.dtype)), tf.reduce_sum(tf.cast(valid, hard.dtype)))
    ...
    tail_count = tf.maximum(
        1,
        tf.cast(
            tf.math.ceil(0.25 * tf.cast(positive_count, tf.float32)), tf.int32
        ),
    )
    ...
    teacher_delta = teacher_decision[:, None] - teacher_decision[None, :]
    student_delta = canonical[:, None] - canonical[None, :]
    comparable = tf.logical_and(
        sequence_teacher_mask[:, None] > 0.5,
        sequence_teacher_mask[None, :] > 0.5,
    )
    comparable = tf.logical_and(
        comparable, tf.equal(labels[:, None] > 0.5, labels[None, :] > 0.5)
    )
    comparable = tf.logical_and(comparable, tf.abs(teacher_delta) >= 0.05)
    teacher_rank_weight = tf.minimum(tf.abs(teacher_delta), 1.0)
    sequence_teacher_loss = _masked_mean(
        tf.nn.softplus(-tf.sign(teacher_delta) * student_delta)
        * teacher_rank_weight,
        comparable,
    )
...
def _student_scores(
    ...
    *,
    ...
    scores = []
    for start in range(0, len(features), batch_size):
        logits = np.asarray(model(np.asarray(features[start : start + batch_size], dtype=np.float32), training=False))
...
def main() -> int:
    ...
    parser.add_argument("--noise-source", action="append", required=True, help="ID=RaggedMmap directory")
    ...
    parser.add_argument(
        "--allow-partial-expanded-public-coverage", action="store_true"
    )
    ...
    clip, continuous = require_teacher_gates(args.teacher_qualification, args.continuous_qualification)
    ...
    student_times = student_output_times_seconds(student_flags(len(contract["tokens"])), OUTPUT_FRAMES)
    teacher_targets = np.empty((len(rows), OUTPUT_FRAMES, len(contract["tokens"])), dtype=np.float32)
    ...
    for index in range(len(rows)):
        teacher_targets[index] = resample_log_posteriors(
            matrix[offsets[index] : offsets[index + 1]],
    ...
    expanded_meta, expanded_public_negative_features = (
        load_expanded_public_negatives(
    ...
    mapped_overlay_targets = map_ordered_targets(raw_overlay_targets, overlay_provenance, contract)
    overlay_parent_binding = require_overlay_parent_binding(rows, overlay_provenance)
    train_ledger = [row for row in overlay_provenance["examples"] if row["split"] == "train"]
    if len(train_ledger) != len(overlay_features) or len(train_ledger) != len(raw_overlay_targets):
        raise ValueError("overlay feature order differs from provenance")
    overlay_indexes = [i for i, row in enumerate(train_ledger) if row.get("variant") != "clean"]
    ...
    def train_batch(
        ...
        with tf.GradientTape() as tape:
            ...
            loss, parts = distillation_loss(
                logits, hard, soft, soft_mask, label, contract,
                ...
                negative_score_target=float(clip["validation_operating_point"]["threshold"]),
    ...
    validation_positive = np.asarray(features[[i for i, row in enumerate(rows) if row["split"] == "validation" and row["label"] == 1]], dtype=np.float32)
    validation_negative_indexes = [i for i, row in enumerate(rows) if row["split"] == "validation" and row["label"] == 0]
    validation_negative = np.asarray(features[validation_negative_indexes], dtype=np.float32)
    validation_seconds = sum(float(rows[i]["duration_seconds"]) for i in validation_negative_indexes)
    args.output.mkdir(parents=True, exist_ok=True)
    checkpoint_directory = args.output / "checkpoints"
    checkpoint_directory.mkdir(exist_ok=True)
    ledger = []
    finite_floor = -np.finfo(np.float64).max
    best_key = (-1.0, -1.0, -1.0, finite_floor)
    best_step = None
    for step in range(args.steps):
        ...
        if (step + 1) % args.eval_every == 0 or step == 0:
            ...
            key = (
                ...
                separation if separation is not None else finite_floor,
            )
            if key > best_key:
                best_key = key
                best_step = step + 1
    ...
    expected_provider_counts, expected_negative_counts, expected_positive_variant_provider_counts = expected_schedule_counts(args.steps, args.batch_size)
    if (
        not args.allow_partial_expanded_public_coverage
        and expected_negative_counts["public_speech"]
        < len(expanded_public_negative_features)
    ):
        raise ValueError(
            "configured run cannot cover every expanded public negative"
        )
    if batcher.provider_counts != expected_provider_counts:
        raise ValueError(f"realized positive provider schedule drifted: {batcher.provider_counts}")
    if batcher.negative_group_counts != expected_negative_counts:
        raise ValueError(f"realized negative group schedule drifted: {batcher.negative_group_counts}")
    if (
    ...
    if batcher.positive_variant_provider_counts != expected_positive_variant_provider_counts:
        raise ValueError(
    ...
    provider_shares = {key: value / provider_total for key, value in sorted(batcher.provider_counts.items())}
    if set(provider_shares) != set(APPROVED_PROVIDERS) or any(not 0.24 <= share <= 0.26 for share in provider_shares.values()):
        raise ValueError(f"realized positive provider balance failed: {provider_shares}")
    metadata = {
        "schema_version": 1,
        "recipe": "kizz_control_compact_ctc_distillation_v6",
        "teacher_qualification": {"path": str(args.teacher_qualification.resolve()), "sha256": sha256_file(args.teacher_qualification)},
        "continuous_qualification": {"path": str(args.continuous_qualification.resolve()), "sha256": sha256_file(args.continuous_qualification)},
        ...
        "noise_sources": [
            {"id": name, "artifact": provenance_ref(path)} for name, path in noise_sources
        ],
        ...
        "loss_contract": {
            ...
            "legacy_negative_score_target": float(clip["validation_operating_point"]["threshold"]),
            "sequence_teacher_weight": args.sequence_teacher_weight,
            "ranking_weight": args.ranking_weight,
            "tail_ranking_weight": args.tail_ranking_weight,
            "ranking_margin": args.ranking_margin,
            ...
            "teacher_sequence_transfer": "within_label_pairwise_rank",
        ...
        "realized_positive_provider_counts": dict(sorted(batcher.provider_counts.items())),
        ...
        "realized_negative_group_counts": dict(sorted(batcher.negative_group_counts.items())),
        "realized_hard_negative_counts": dict(sorted(batcher.hard_negative_counts.items())),
        "realized_hard_positive_counts": dict(sorted(batcher.hard_positive_counts.items())),
        "realized_noise_source_counts": dict(sorted(batcher.noise_source_counts.items())),
        "sampling_contract": {
            "seed": args.seed,
            "steps": args.steps,
            "batch_size": args.batch_size,
            ...
            "negative_group_order": ["public_speech", "kizz_control_phonetic_collision", "device_collision", "no_speech"],
            "noise_source_order": [name for name, _ in noise_sources],
            "expected_positive_provider_counts": dict(sorted(expected_provider_counts.items())),
            "expected_positive_variant_provider_counts": nested_variant_provider_counts(
                expected_positive_variant_provider_counts
            ),
            "expected_negative_group_counts": dict(sorted(expected_negative_counts.items())),
        },
        "best_checkpoint_contract": {
            "selection_key": ["qualified", "zero_false_accept_recall", "recall", "separation"],
            "tie_break": "earliest_evaluation_step",
            "evaluation_interval_steps": args.eval_every,
            "evaluation_geometry": "all_streaming_window_endpoints_hop_1",
        },
