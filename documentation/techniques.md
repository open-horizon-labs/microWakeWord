...
| Human-referenced synthetic quality mask | Compare generated duration, voiced span, and level with reviewed human positives; reject silence, clipping, implausible positive spans, and clips at risk of training-window truncation; bind the decision to corpus and recipe hashes. | [`microwakeword/synthetic_quality.py`](../microwakeword/synthetic_quality.py), [`tools/build_synthetic_quality_mask.py`](../tools/build_synthetic_quality_mask.py) | Fork data-quality control. Default positive limits use the human 5th and 95th span percentiles with a 25% margin; hard negatives keep their natural range. |
