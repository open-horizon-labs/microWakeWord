...
| Human-referenced synthetic quality mask | Compare generated duration and voiced span with reviewed human positives, reject silence/clipping and clips at risk of training-window truncation, and bind the mask to corpus and recipe hashes. | [`microwakeword/synthetic_quality.py`](../microwakeword/synthetic_quality.py), [`tools/build_synthetic_quality_mask.py`](../tools/build_synthetic_quality_mask.py) | Fork data-quality control; recorded phrase spans are the empirical reference rather than an assumed wake duration. |
